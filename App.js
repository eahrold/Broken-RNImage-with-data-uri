...
const App: () => React$Node = () => {
  ...
  const toggle = () => {
    if (imgType === 'gif') setImgType('png');
    else setImgType('gif');
  };

  const uri = images[imgType];
  ...
  return (
    <View style={styles.body}>
      ...
      <Button title="Break Image data Gif with Alert" onPress={onPress} />
      <Button title="Toggle Broken Image" onPress={() => setHidden(!hidden)} />

      <View style={[styles.body, {flex: 0, padding: 40}]}>
        <Button title="Switch Source" onPress={() => toggle()} />
        <Text style={styles.ypadding}>URI Length: {uri.length}</Text>
        <Text style={styles.ypadding}>{uri.substr(0, 100)}...</Text>
      </View>
    </View>
  );
};
